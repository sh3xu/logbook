...
export default function Logbook() {
  ...
  const [decryptionStatusMap, setDecryptionStatusMap] = useState<Record<string, EntryStatus>>({})
  ...
  useEffect(() => {
    const fetchEntries = async () => {
      ...
      if (error) {
        console.error("Error fetching entries:", error.message, error.code, error.details)
      } else if (data) {
        ...
        const processedEntries: DailyEntry[] = await Promise.all(
          data.map(async (dbEntry: any) => {
            ...
            if (dbEntry.is_encrypted) {
              if (encryptionKey) {
                try {
                  const decryptedStr = await decryptMessage(dbEntry.content, encryptionKey)
            ...
            return {
              ...
              classification: entryData.classification || { work: 0, personal: 0, health: 0, learning: 0 },
            } as DailyEntry
          })
        )
  ...
  return (
    <div className="p-6 space-y-6 bg-white h-full overflow-y-auto selection:bg-black selection:text-white">
      ...
      <div className="border-[3px] border-black p-4 bg-white shadow-[6px_6px_0_0_#000]">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-black text-sm uppercase tracking-widest">{monthName}</h3>
          </div>
          <div className="flex gap-2">
            ...
            <button onClick={() => setViewDate(new Date(viewDate.getFullYear(), viewDate.getMonth() - 1))} className="p-2 border-[2px] border-black hover:bg-black hover:text-white transition-all"><ChevronLeft className="w-4 h-4" /></button>
            <button onClick={() => setViewDate(new Date(viewDate.getFullYear(), viewDate.getMonth() + 1))} className="p-2 border-[2px] border-black hover:bg-black hover:text-white transition-all"><ChevronRight className="w-4 h-4" /></button>
          </div>
        </div>

        <div className="grid grid-cols-7 gap-1">
          {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => <div key={`${d}-${i}`} className="text-center font-black text-[10px] p-2 bg-black text-white">{d}</div>)}
          {Array.from({ length: firstDay }).map((_, i) => <div key={i} className="p-2 bg-gray-50 border-[2px] border-dashed border-gray-200 h-14" />)}
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            const isBeforeJoin = joinDate ? new Date(date.setHours(23,59,59,999)) < new Date(joinDate.setHours(0,0,0,0)) : false
            ...
            return (
              ...
              >
                <span className={`text-[10px] font-black ${isToday ? 'underline decoration-[2px]' : ''}`}>{day}</span>
                <div className="mt-1">
                  {status === "decrypted" && <CheckCircle2 size={12} className={isSelected ? "text-green-400" : "text-green-600"} />}
                  {status === "encrypted" && <Lock size={12} className={isSelected ? "text-yellow-300" : "text-yellow-600"} />}
                  {status === "corrupted" && <AlertCircle size={12} className={isSelected ? "text-red-300" : "text-red-500"} />}
                  {status === "missed" && <Clock size={10} className="text-gray-300" />}
                </div>
              </button>
            )
          })}
      ...
      <div className="flex items-center justify-between border-b-[3px] border-black pb-2">
        <div className="font-black uppercase tracking-tighter text-lg">{selectedDate === getTodayDateString() ? "Today's Logs" : selectedDate}</div>
      </div>
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="flex flex-col items-center py-12 gap-3">
            ...
            <span className="font-black uppercase text-[10px] tracking-widest">Accessing Vault...</span>
          </div>
            ...
            {getDayStatus(parseInt(selectedDate.split("-")[2])) === "missed" ? (
              <>
                ...
                <div className="text-center">
                  <p className="font-black uppercase text-xs text-red-400">Missed Connection</p>
                  <p className="text-[10px] font-bold text-gray-400 italic">No logs found for this cycle.</p>
                </div>
              </>
            ) : (
              <>
                ...
                <div className="text-center">
                  <p className="font-black uppercase text-xs">Zero Data Points</p>
                  <p className="text-[10px] font-bold text-gray-400 italic">Select a marked day or create a new entry.</p>
                </div>
              </>
            )}
          </div>
          ...
          filteredEntries.map((entry) => (
            <div key={entry.id} className="border-[3px] border-black bg-white shadow-[8px_8px_0_0_#000] overflow-hidden">
              <div
                role="button"
                tabIndex={0}
              ...
              >
                <div className="flex items-center gap-6">
                  <span className="text-5xl drop-shadow-[2px_2px_0_rgba(0,0,0,1)]">{entry.mood}</span>
                  <div className="flex flex-col items-start w-full">
                    <div className="flex items-center justify-between w-full">
                      <div className="text-[10px] font-black uppercase text-gray-400 tracking-widest flex items-center gap-1">
                        {decryptionStatusMap[entry.date] === "decrypted" ? <Unlock size={10} /> : <Lock size={10} />}
                        ...
                        <button 
                          onClick={(e) => {
                            ...
                            setModal({
                              isOpen: true,
                              title: "Purge Entry",
                              message: "Are you sure you want to delete today's entry? This protocol is irreversible.",
                              type: "confirm",
                    ...
                    <div className="text-sm font-black uppercase tracking-tight overflow-hidden text-ellipsis whitespace-nowrap max-w-[200px] md:max-w-md opacity-80 mt-1">
                      {entry.logText.slice(0, 45)}{entry.logText.length > 45 ? "..." : ""}
                    </div>
                  </div>
                </div>
                ...
                <div className="p-6 border-t-[3px] border-black bg-white grid grid-cols-1 md:grid-cols-2 gap-6 animate-in slide-in-from-top-2 duration-200">
                  <div className="space-y-6">
                    <section>
                      <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-[#FF2E63] mb-3 flex items-center gap-2">
                        <div className="w-2 h-2 bg-[#FF2E63]" /> {decryptionStatusMap[entry.date] === "corrupted" ? "Corrupted Stream" : "Decrypted Stream"}
                      </h4>
                      {decryptionStatusMap[entry.date] === "corrupted" ? (
                        <div className="p-4 border-[2px] border-black bg-red-50 space-y-3">
                          <p className="text-[10px] font-bold text-red-600 italic">This entry was encrypted with a different key or data is corrupted.</p>
                          <div className="flex gap-2">
                            <input 
                              type="password" 
                            ...
                            <button 
                              onClick={async () => {
                                const k = (document.getElementById(`retry-key-${entry.id}`) as HTMLInputElement).value
                                if (!k) return
                                try {
                                  // Attempt one-off decryption
                                  const content = entries.find(e => e.id === entry.id)?.logText || ""
                                  ...
                                  setModal({
                                    ...
                                    message: "Please refresh your data stream to attempt re-decryption with the global vault key.",
                                    type: "alert",
                          ...
                          <button className="text-[9px] font-black uppercase text-gray-500 hover:underline">View Encrypted Payload</button>
                        </div>
                      ) : (
                        <p 
                    ...
                    <section className="grid grid-cols-2 gap-4">
                      <div className="bg-green-50/50 p-3 border-[2px] border-black">
                        <h4 className="text-[10px] font-black uppercase tracking-widest text-green-700 mb-2">High Points</h4>
                        <ul className="space-y-1.5">
                          {entry.achievements.length > 0 ? (
                            entry.achievements.map((a, i) => <li key={i} className="text-[10px] font-bold p-1 border-b-[1px] border-green-200 last:border-0">• {a}</li>)
                          ) : (
                            <li className="text-[10px] text-gray-400 italic">No achievements logged.</li>
                          )}
                        </ul>
                      </div>
                      <div className="bg-red-50/50 p-3 border-[2px] border-black">
                        <h4 className="text-[10px] font-black uppercase tracking-widest text-red-700 mb-2">Friction</h4>
                        <ul className="space-y-1.5">
                          {entry.challenges.length > 0 ? (
                            entry.challenges.map((c, i) => <li key={i} className="text-[10px] font-bold p-1 border-b-[1px] border-red-200 last:border-0">• {c}</li>)
                          ) : (
                            <li className="text-[10px] text-gray-400 italic">No challenges logged.</li>
                          )}
                        </ul>
                      </div>
                    </section>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between border-[3px] border-black p-4 bg-black text-white shadow-[4px_4px_0_0_#FF2E63]">
                      <span className="text-xs font-black uppercase tracking-widest">Time Injected</span>
                      <span className="text-2xl font-black">{entry.workingHours}H</span>
                    </div>
                  </div>
                </div>
              )}
