            ...
            <div className="border-[3px] border-black p-6 bg-[#FAFAF0] shadow-[6px_6px_0_0_#000]">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-4 bg-white border-[3px] border-black flex flex-col items-center">
                  ...
                  <span className="text-xl font-black text-[#FF2E63]">
              ...
              <div className="p-12 border-[3px] border-black border-dashed flex flex-col items-center justify-center text-center gap-4 bg-gray-50">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-gray-400 leading-relaxed italic">
                    Log details are privately encrypted. Only shared metrics are
    ...
    <div className="p-4 md:p-6 bg-white h-full flex flex-col selection:bg-black selection:text-white">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-1">
          <button
            ...
            className={`p-2 border-[2px] border-black transition-all ${activeTab === "friends" ? "bg-black text-white" : "bg-white hover:bg-gray-50"}`}
          >
            <Users size={16} />
          </button>
          <button
            onClick={() => setActiveTab("requests")}
            className={`p-2 border-[2px] border-black transition-all ${activeTab === "requests" ? "bg-black text-white shadow-[2px_2px_0_0_#FF2E63]" : "bg-white hover:bg-gray-50"}`}
          >
            <UserPlus size={16} />
          </button>
          <button
            onClick={() => setActiveTab("search")}
            className={`p-2 border-[2px] border-black transition-all ${activeTab === "search" ? "bg-black text-white" : "bg-white hover:bg-gray-50"}`}
          >
            <Search size={16} />
          </button>
        </div>
          ...
          {friends.length === 0 ? (
            <div className="h-40 border-[3px] border-black border-dashed flex items-center justify-center text-center p-6 bg-gray-50">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-widest leading-loose">
                ...
                <br />
                Search users to populate your grid.
              </p>
            </div>
          ) : (
            friends.map((friend) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <div className="flex flex-col items-end">
                    ...
                    <span className="text-sm font-black text-[#FF2E63]">
                      🔥{friend.currentStreak}
                    </span>
                  </div>
                  <ChevronRight
          ...
          {friendRequests.length === 0 ? (
            <div className="h-40 border-[3px] border-black border-dashed flex items-center justify-center text-center p-6 bg-gray-50">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-widest italic">
                All communication channels clear.
              </p>
            </div>
          ) : (
            friendRequests.map((req) => (
              <div
                key={req.id}
                className="p-4 border-[3px] border-black bg-blue-50 flex items-center justify-between gap-4 animate-in slide-in-from-right-4"
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 border-[2px] border-black bg-black text-white flex items-center justify-center font-black">
                    {req.avatar}
                  </div>
                  <div>
                    ...
                    <div className="text-[9px] font-bold text-gray-500 italic">
                      Incoming Signal
                    </div>
                  </div>
                </div>
                <div className="flex gap-2">
                  <button
                    ...
                    className="p-2 border-[2px] border-black bg-white hover:bg-green-500 hover:text-white transition-all"
                  >
                    <Check size={16} />
                  </button>
                  <button className="p-2 border-[2px] border-black bg-white hover:bg-red-500 hover:text-white transition-all">
                    <X size={16} />
                  </button>
                </div>
              </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-3">
            {loading ? (
              ...
              searchResults.map((user) => (
                <div
                  key={user.id}
                  className="p-4 border-[3px] border-black bg-white flex items-center justify-between gap-4 hover:translate-x-1 transition-transform"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 border-[2px] border-black bg-black text-white flex items-center justify-center font-black">
              ...
              searchQuery && (
                <p className="text-center text-[10px] font-black text-gray-400 uppercase italic">
      ...
      <div className="mt-8 p-4 border-[2px] border-black bg-[#FAFAF0] flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest">
            {friendRequests.length} Pending Signals
          </span>
        </div>
      </div>
    </div>
