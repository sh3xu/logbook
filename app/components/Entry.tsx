...
export default function Entry() {
  ...
  const [formData, setFormData] = useState<{
    ...
    const baseMetrics = lastEntry
      ? Object.keys(lastEntry.classification)
      : metricDefinitions;
    ...
    if (formData.mood && formData.classification) {
      if (!encryptionKey) {
        setModal({
          ...
          title: "Vault Locked",
          message:
            "You must unlock your vault with the Master Key before saving an entry. This ensures your data remains encrypted and secure.",
          type: "alert",
      ...
      if (
        JSON.stringify(currentMetricKeys) !== JSON.stringify(metricDefinitions)
      ) {
    ...
    <div className="p-4 md:p-6 space-y-4 bg-white h-full overflow-y-auto">
          ...
          {moodOptions.map((m) => (
            <button
              ...
              className={`text-3xl p-2 border-[3px] transition-all ${
                formData.mood === m
                  ? "border-black scale-110 shadow-[3px_3px_0_0_#000]"
                  : "border-gray-300"
              }`}
            >
              {m}
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) =>
            setFormData({ ...formData, workingHours: parseInt(e.target.value) })
          }
            ...
            onClick={() => {
              ...
              if (current.length > 0 && !current[current.length - 1].trim()) {
                addNotification({
                  ...
                  type: "streak",
                  message:
                    "Please fill the current achievement before adding another.",
                  timestamp: "just now",
                  read: false,
                });
                return;
            ...
            onClick={() => {
              ...
              if (current.length > 0 && !current[current.length - 1].trim()) {
                addNotification({
                  ...
                  type: "streak",
                  message:
                    "Please fill the current challenge before adding another.",
                  timestamp: "just now",
                  read: false,
                });
                return;
      ...
      <div className="border-t-[2px] border-black pt-4">
          ...
          <button
            onClick={() => {
              setModal({
                ...
                title: "New Metric",
                message:
                  "Enter the name of the productivity or life metric you want to track:",
                type: "prompt",
                onConfirm: (name) => {
        ...
        <div className="space-y-4 bg-gray-50 p-4 border-[2px] border-black">
          {Object.entries(formData.classification).map(([key, value]) => (
            <div key={key} className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono font-black bg-white border-[2px] border-black px-3 py-1 text-xs">
                  {value}%
                </span>
              </div>
              <input
          ...
          {Object.keys(formData.classification).length === 0 && (
            <p className="text-[10px] font-bold text-gray-400 text-center py-4 italic">
      ...
      <div className="flex gap-3 pt-4 border-t-[2px] border-black mt-6">
        ...
        >
          {entries.some((e) => e.date === getTodayDateString())
