...
import { MessageSquare, Plus, Loader2, CheckCircle2, Clock, PlayCircle, XCircle, Trash2, Filter as FilterIcon, AlertTriangle } from "lucide-react"
...
export default function FeatureRequests() {
  ...
  const [matcher] = useState(() => new RegExpMatcher({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (lastRequestDate === today && !profile.isAdmin) {
      setModal({
        ...
        message: "You can only submit one feature request per day. Quality over quantity!",
        type: "alert",
    ...
    const cleanTitle = matcher.hasMatch(newTitle) 
      ? newTitle.replace(/\b\w+\b/g, (word) => matcher.hasMatch(word) ? '****' : word)
      : newTitle
    const cleanDesc = matcher.hasMatch(newDesc)
      ? newDesc.replace(/\b\w+\b/g, (word) => matcher.hasMatch(word) ? '****' : word)
      : newDesc
  ...
  const deleteRequest = async (id: string) => {
    setModal({
      ...
      message: "Are you sure you want to permanently delete this feature request from the database?",
      type: "confirm",
      onConfirm: async () => {
        const { data, error } = await supabase
          .from("feature_requests")
  ...
  const getStatusIcon = (status: Request['status']) => {
    switch (status) {
      ...
      case 'in-progress': return <PlayCircle className="text-blue-500 w-4 h-4" />
      case 'planned': return <Clock className="text-yellow-500 w-4 h-4" />
  ...
  const filteredRequests = statusFilter === 'all' 
    ? requests.filter(r => profile.isAdmin || !['pending', 'rejected'].includes(r.status))
    : requests.filter(r => r.status === statusFilter)

  const availableFilters = profile.isAdmin 
    ? (['all', 'pending', 'planned', 'in-progress', 'completed', 'rejected'] as const)
    ...
    <div className="p-6 space-y-8 bg-white max-h-[600px] overflow-y-auto selection:bg-black selection:text-white">
      {!profile.isAdmin && (
        <div className="space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 p-4 border-[3px] border-black bg-yellow-50 shadow-[4px_4px_0_0_#000]">
            <div className="space-y-2">
              <label className="text-xs font-black uppercase tracking-widest text-black/50">Feature Title</label>
              <input
                type="text"
                value={newTitle}
            ...
            <div className="space-y-2">
              <label className="text-xs font-black uppercase tracking-widest text-black/50">Description (Optional)</label>
              <textarea
                value={newDesc}
            ...
            {profanityWarning && (
              <div className="p-3 border-2 border-red-500 bg-red-50 flex items-center gap-2">
                ...
                <p className="text-xs font-black uppercase text-red-600">Inappropriate language detected. Words will be censored on submit.</p>
              </div>
            )}
            <button
              type="submit"
            ...
            >
              {submitting ? <Loader2 className="w-5 h-5 animate-spin" /> : isLocked ? <Clock className="w-5 h-5" /> : <Plus className="w-5 h-5" />}
      ...
      <div className="space-y-6">
        ...
        {loading ? (
          ...
          <p className="text-sm font-bold text-black/40 italic">No {statusFilter !== 'all' ? statusFilter : ''} requests found.</p>
        ) : (
          <div className="space-y-4">
            {filteredRequests.map(req => (
              <div 
                ...
                className={cn(
                  ...
                  profile.isAdmin ? "bg-indigo-50 border-indigo-900 shadow-indigo-900/20" : "bg-white"
                )}
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-1">
                    <h3 className="font-black text-lg uppercase leading-tight">{req.title}</h3>
                    {req.description && <p className="text-sm font-bold text-gray-600">{req.description}</p>}
                  </div>
                ...
                {profile.isAdmin && (
                  <div className="pt-3 border-t-2 border-black/10 flex flex-wrap gap-2">
                    {(['pending', 'planned', 'in-progress', 'completed', 'rejected'] as const).map(s => (
                      <button
                        key={s}
